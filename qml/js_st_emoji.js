...
var EMOJI_MAP = {
  ...
  for (var i=0;i<unicodelist.length;i++){
    ...
    if (reg.test(strtext)){
     ...
     strraw=strraw.replace(reg,vstr);
     emojicount=emojicount+1;
    }
  }
    return strtext+"@*@"+strraw+"@*@"+emojicount;
}
function countStrLength(str,ecount){
  var num=0;
  var chnum=0;
  var strlen=0;
  var cntlen=0;
  ...
  for(var i=0;i<str.length;i++){
    var count=checkChinese(str.charAt(i));
    if (count==0){
        chnum=chnum+1;
    } else{
        num=num+1;
  ...
  {
    strlen=cntlen*15+(ecount-1)*10;
    if (ecount<=7) strlen=strlen+15;
    mh=30;
  }
  //如果有emoji，1个得补30;
  if (strlen>260){
    var ch=Math.ceil((cntlen*15)/260);
    if(ecount==0){
      mh=ch*15.5;
    }
     else{
  ...
  console.log("c++,emoji__countStrLength:"+mh);
  return strlen+"@@"+mh;
}
