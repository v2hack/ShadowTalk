...
public:
    ...
    virtual int send_message_via_direct_connection(const std::string &connection_id,
                                                   ...
                                                   unsigned long message_id, const std::string &data) = 0;
    /* 底层通知上层绑定connection_id和friend_channel_id */
    virtual void bind_friend_channel(const std::string &connection_id,
...
public:
    ...
    void direct_connection_message_received(const string &connection_id,
                                            const string &friend_channel_id,
                                            const string &data);
    /* 向附近的一个连接的发送好友请求 */
    std::string add_friend_via_direct_connection(const string &connection_id,
